...
const l = r.z.object({
  ...
  f = new Set(["linh_thach", "Hạ Phẩm Linh Thạch"]);
function p(r) {
  return String(r ?? "")
    .trim()
    .replace(/\s+/g, "");
...
const v = r.z
    .object({
      ten: r.z.string().prefault(""),
      mo_ta: r.z.string().prefault(""),
      loai: r.z.enum(["cong_phap", "Thần Thông", "Bí Thuật"]).prefault("Thần Thông"),
      pham_cap: r.z
        .string()
...
const D = r.z.object({
    ...
    cong_phap: r.z.string().describe("cong_phap"),
...
const Y = r.z
  ...
  sr = r.z.object({
    ...
    loai: r.z.enum(["Bí Cảnh", "Thành Trấn", "tong_mon", "Cấm Địa", "Di Tích", "Địa Hình"]),
      ...
      kho_dia_diem: r.z
        ...
        .prefault({
          ...
          "Vấn Đạo Phong": {
            ...
            nguy: 10,
            dac: "Vạn Pháp Tông",
            tai_nguyen: ["cong_phap", "Linh Dược"],
          },
          "Kiếm Môn Quan": {
            vuc: "Thần Châu",
            loai: "tong_mon",
...
function Ar(r) {
  ...
  if (t && jr.has(t[1]))
    return r.replace(`stat_data.hong_nhan.${t[1]}`, "stat_data.hong_nhan.Ngu Tịch Nhan");
  const e = r.match(/^stat_data\.kho_nhan_vat_hong_nhan\.([^./]+)(?=\.|$)/);
  return e && jr.has(e[1])
    ? r.replace(`stat_data.kho_nhan_vat_hong_nhan.${e[1]}`, "stat_data.kho_nhan_vat_hong_nhan.Ngu Tịch Nhan")
    : r;
}
function $r(r, t) {
