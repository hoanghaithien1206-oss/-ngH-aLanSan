...
const Y = r.z
  ...
  ur = [
    ...
    { canonical: "soc_ly_oanh", aliases: ["Sóc Ly Oanh", "Sóc Ly Diên", "A Oanh", "Huyết Thủ Phi Oanh"] },
    ...
    .transform((r) => {
      r.kho_nhan_vat_hong_nhan = (function (r) {
        ...
        for (const item of ur) {
          nameToCanonical.set(p(item.canonical).toLowerCase(), item.canonical);
          for (const alias of item.aliases) {
            nameToCanonical.set(p(alias).toLowerCase(), item.canonical);
          }
        }
        for (const key of Object.keys(t)) {
          const normKey = p(key).toLowerCase();
          const canonical = nameToCanonical.get(normKey);
          if (canonical && canonical !== key) {
            t[canonical] = gr(t[canonical], t[key]);
            delete t[key];
          }
        }
        return t;
      ...
      const t = (function (r, t) {
        ...
        for (const key of Object.keys(e)) {
          ...
          const canonical = nameToCanonical.get(normKey);
          if (canonical && canonical !== key) {
            e[canonical] = zr(e[canonical], e[key]);
            delete e[key];
          }
        }
        for (const key of Object.keys(n)) {
          ...
          const canonical = nameToCanonical.get(normKey);
          if (canonical && canonical !== key) {
            const o = [n[canonical], n[key]]
              .map((r) => Number(r))
              .filter((r) => Number.isFinite(r));
            delete n[key];
            if (o.length > 0) {
              n[canonical] = o[o.length - 1];
            }
          }
        }
...
ur.forEach(item => {
  aliasMap.set(p(item.canonical).toLowerCase(), item.canonical);
  item.aliases.forEach(alias => aliasMap.set(p(alias).toLowerCase(), item.canonical));
});
function Ar(r) {
  const t = r.match(/^stat_data\.hong_nhan\.([^./]+)(?=\.|\$)/);
  if (t) {
    const nameKey = p(t[1]).toLowerCase();
    if (aliasMap.has(nameKey)) {
      return r.replace(`stat_data.hong_nhan.${t[1]}`, `stat_data.hong_nhan.${aliasMap.get(nameKey)}`);
    }
  }
  const e = r.match(/^stat_data\.kho_nhan_vat_hong_nhan\.([^./]+)(?=\.|\$)/);
  if (e) {
    const nameKey = p(e[1]).toLowerCase();
    if (aliasMap.has(nameKey)) {
      return r.replace(`stat_data.kho_nhan_vat_hong_nhan.${e[1]}`, `stat_data.kho_nhan_vat_hong_nhan.${aliasMap.get(nameKey)}`);
    }
  }
  return r;
}
  ...
  r[hr] ||
    ...
    eventOn(Mvu.events.COMMAND_PARSED, (r, t) => {
      for (const e of t) {
        ...
        const isStatData = t.startsWith("/stat_data") || t.startsWith("stat_data");
        if (Dr(e, n, r)) continue;
        const a = xr(e);
        if (null !== a && e.args.length > a) {
          const r = e.args[a],
            val = $r(n, r);
          (_.isEqual(r, val) || (e.args[a] = val), wr(n, val));
        }
        let finalPath = n;
        if (isSlash) {
          if (!isStatData && finalPath.startsWith("stat_data.")) {
            finalPath = finalPath.slice("stat_data.".length);
          }
