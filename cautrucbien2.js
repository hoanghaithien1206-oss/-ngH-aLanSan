...
const _r = e.z
  ...
  .transform((r) => {
    if (r.hong_nhan && r.kho_nhan_vat_hong_nhan) {
      for (const canonical of Object.keys(r.kho_nhan_vat_hong_nhan)) {
        const stripped = canonical.replace(/\s+/g, "");
        if (stripped !== canonical && r.hong_nhan[stripped] && !r.hong_nhan[canonical]) {
          r.hong_nhan[canonical] = r.hong_nhan[stripped];
          delete r.hong_nhan[stripped];
        }
      }
    }
    if (r.snapshot_do_hao_cam && r.kho_nhan_vat_hong_nhan) {
      for (const canonical of Object.keys(r.kho_nhan_vat_hong_nhan)) {
        const stripped = canonical.replace(/\s+/g, "");
        if (stripped !== canonical && r.snapshot_do_hao_cam[stripped] !== undefined && r.snapshot_do_hao_cam[canonical] === undefined) {
          r.snapshot_do_hao_cam[canonical] = r.snapshot_do_hao_cam[stripped];
          delete r.snapshot_do_hao_cam[stripped];
        }
      }
    }
